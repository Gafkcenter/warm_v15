...
public class JedisUtils {
	...
	static {
		...
		// redis如果设置了密码：
		 ...
		 jedisPool = new JedisPool(config, "127.0.0.1",6379, 10000,"gafker619");
...
//			jedisPool.returnResource(jedis);
	...
	public static void main(String[] args) {
		...
		//2.使用原生操作方法操作jedis
		//通过RedisUtils实例获取Jedis连接对象；这样就可以用原生的方式使用；最后使用完后需要手动将其归还到池子中：
	...
	private static void saveObject() {
		JedisUtils jedisUtil = JedisUtils.getInstance();
		Userattribute p = new Userattribute();  
        p.setId(3l);  
        p.setUsername("测试");  
        ...
        Userattribute p1 = (Userattribute) SerializeUtil.unserialize(personBy);  
