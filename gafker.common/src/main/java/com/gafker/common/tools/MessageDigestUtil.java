...
public class MessageDigestUtil {
   ...
   public static String base64HashByMD5(String password, String salt) {
	 ...
	 * String salt = UUID.randomUUID().toString();
			...
			String pass = MessageDigestUtil.base64HashBySHA256(user.getPassword(), salt);
			user.setPassword(pass);
	 */
	/**
	 * Demo验证密码：
	 * List<CommonUser> list = commonUserMapper.selectByExample(us);
		if (list.size() > 0) {
			CommonUser user = list.get(0);
			String dbpass = user.getPassword();
			String salt = user.getPswSalt();

			String pass = MessageDigestUtil.base64HashBySHA256(password, salt);
